...
//
//  +-----------+- . . . -+-----------+-------------+-------------+
//  | Orig Size |  Data   | File name | Line number |     CPU     |
//  +-----------+- . . . -+-----------+-------------+-------------+
...
//  - CPU (uint32_t) is the CPU that requested the memory.
//
...
#define kmalloc(size) \
    kmalloc_debug_wrapper(__FILE__, __LINE__, size)
...
#define kfree(addr) \
    kfree_debug_wrapper(__FILE__, __LINE__, addr)
